...
if (isset($_GET['cat'])) {
    $cat_url = '&amp;cat=' . $_GET['cat'];
    $cat_url_alt = '?cat=' . $_GET['cat'];
} else {
    $cat_url = '';
    $cat_url_alt = '';
}
...
if (isset($working_days_obj) && !empty($working_days_obj)) {
    ...
    if (isset($working_days_arr[0])) {
        $Mon = $working_days_arr[0];
    }
    if (isset($working_days_arr[1])) {
        $Tue = $working_days_arr[1];
    }
    if (isset($working_days_arr[2])) {
        $Wed = $working_days_arr[2];
    }
    if (isset($working_days_arr[3])) {
        $Thu = $working_days_arr[3];
    }
    if (isset($working_days_arr[4])) {
        $Fri = $working_days_arr[4];
    }
    if (isset($working_days_arr[5])) {
        $Sat = $working_days_arr[5];
    }
    if (isset($working_days_arr[6])) {
        $Sun = $working_days_arr[6];
    }

    if (empty($working_days_arr[0])) {
        $Mon = '1&&0&&0';
    }
    if (empty($working_days_arr[1])) {
        $Tue = '1&&0&&0';
    }
    if (empty($working_days_arr[2])) {
        $Wed = '1&&0&&0';
    }
    if (empty($working_days_arr[3])) {
        $Thu = '1&&0&&0';
    }
    if (empty($working_days_arr[4])) {
        $Fri = '1&&0&&0';
    }
    if (empty($working_days_arr[5])) {
        $Sat = '1&&0&&0';
    }
    if (empty($working_days_arr[6])) {
        $Sun = '1&&0&&0';
    }
...
if (isset($custom_date_obj) && empty($custom_date_obj) || $custom_date_obj == '&&') {
    ...
    if (isset($custom_date_arr[0])) {
        $date_arr = explode('&&', $custom_date_arr[0]);
        array_pop($date_arr);
    }
    if (isset($custom_date_arr[1])) {
        $dp_arr = explode('&&', $custom_date_arr[1]);
    }
    if (isset($custom_date_arr[2])) {
        $pm_arr = explode('&&', $custom_date_arr[2]);
    }
    if (isset($custom_date_arr[3])) {
        $cprice_arr = explode('&&', $custom_date_arr[3]);
    }
...
if (array($date_arr)) {
    for ($cd = 0; $cd < sizeof($date_arr); ++$cd) {
        ...
        if ($dp_arr[$cd] == 0) {
            if (isset($md_arr[0]) && isset($md_arr[1])) {
                $nwdays_str .= '#' . $md_arr[0] . '.' . $md_arr[1] . '&';
            }
        }
    }//count
}
...
for ($i = 0; $i <= 11; $i++) {
    ...
    $calendar .= '<option value="' . $mn . '" ';
    if ($month == $i + 1) {
        $calendar .= 'selected = "selected"';
    }
    $calendar .= '>' . $months[$i] . '</option>';
...
$action_form = 'http://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];
...
//===========================================================================================
    ...
    if (isset($custom_date_obj) && empty($custom_date_obj) || $custom_date_obj == '&&') {
        ...
        $arr_dates_cd = explode('&&', $arr_cdp[0]);
        array_pop($arr_dates_cd);
        $arr_active_cd = explode('&&', $arr_cdp[1]);
        array_pop($arr_active_cd);
        $arr_sign_cd = explode('&&', $arr_cdp[2]);
        array_pop($arr_sign_cd);
        $arr_prise_cd = explode('&&', $arr_cdp[3]);
        array_pop($arr_prise_cd);

        for ($cdp = 0; $cdp < sizeof($arr_dates_cd); ++$cdp) {
            $arr_dm = explode('.', $arr_dates_cd[$cdp]);


            if ($arr_active_cd[$cdp] != 0) {
                $amays_str .= '#' . $arr_dm[0] . '.' . $arr_dm[1] . '&';
            }
...
// lost busy day
	...
	<div class="clear"></div>
        ...
        if (isset($_GET['day']) && $_GET['day'] == $dd) {
            $a_class = $a_class . ' select_date';
        }
...
//shift wd
                    if ($sign_wd == '+') {
                        $fix_price_obj = $fix_price_obj + $shift_price_wd;
                    }
                    if ($sign_wd == '-' && $fix_price_obj > $shift_price_wd) {
                        $fix_price_obj = $fix_price_obj - $shift_price_wd;
                    }
//shift wd
...
//shift wd
                    if ($sign_wd == '+') {
                        $price_day = $price_day + $shift_price_wd;
                    }
                    if ($sign_wd == '-' && $price_day > $shift_price_wd) {
                        $price_day = $price_day - $shift_price_wd;
                    }
//shift wd
                        ...
                        if ($file_order_bd == '1') {
                            for ($lot = 0; $lot < sizeof($lines_data_order); ++$lot) {
                                if (!empty($lines_data_order[$lot])) {
                                    ...
                                    if (isset($data_ord[0])) {
                                        $id_order_obj = $data_ord[0];
                                    } else {
                                        $id_order_obj = '';
                                    }
                                    if (isset($data_ord[1])) {
                                        $id_obj_obj = $data_ord[1];
                                    } else {
                                        $id_obj_obj = '';
                                    }
                                    if (isset($data_ord[2])) {
                                        $name_ord_obj = $data_ord[2];
                                    } else {
                                        $name_ord_obj = '';
                                    }
                                    if (isset($data_ord[3])) {
                                    ...
                                    if (isset($data_ord[5])) {
                                        $spots_ord_obj = $data_ord[5];
                                    } else {
                                        $spots_ord_obj = '';
                                    }
                                    if (isset($data_ord[6])) {
                                        $client_name_obj = $data_ord[6];
                                    } else {
                                        $client_name_obj = '';
                                    }
                                    if (isset($data_ord[7])) {
                                        $phone_client_obj = $data_ord[7];
                                    } else {
                                        $phone_client_obj = '';
                                    }
                                    if (isset($data_ord[8])) {
                                        $mail_client_obj = $data_ord[8];
                                    } else {
                                        $mail_client_obj = '';
                                    }
                                    if (isset($data_ord[9])) {
                                        $comment_client_obj = $data_ord[9];
                                    } else {
                                        $comment_client_obj = '';
                                    }
                                    if (isset($data_ord[10])) {
                                        $add_ip_obj = $data_ord[10];
                                    } else {
                                        $add_ip_obj = '';
                                    }
                                    if (isset($data_ord[11])) {
                                        $status_obj = $data_ord[11];
                                    } else {
                                        $status_obj = '';
                                    }
                                    if (isset($data_ord[12])) {
                                        $price_ord_obj = $data_ord[12];
                                    } else {
                                        $price_ord_obj = '';
                                    }
                                    if (isset($data_ord[13])) {
                                        $cur_obj = $data_ord[13];
                                    } else {
                                        $cur_obj = '';
                                    }
                                    if (isset($data_ord[14])) {
                                        $order_staff_obj = $data_ord[14];
                                    } else {
                                        $order_staff_obj = '';
                                    }
                                    if (isset($data_ord[15])) {
                                        $discount_ord_obj = $data_ord[15];
                                    } else {
                                        $discount_ord_obj = '';
                                    }
...
//=========================================================STAFF
                ...
                $d_staff_arr = explode('||', $daily_staff_obj);
                array_pop($d_staff_arr);
                ...
                foreach ($d_staff_arr as $kst => $unit_id_staff) {
                    ...
                    for ($lst = 0; $lst < sizeof($lines_staff); ++$lst) {
                        if (!empty($lines_staff[$lst])) {
                            ...
                            if (isset($data_staff[0])) {
                                $id_staff = $data_staff[0];
                            } else {
                                $id_staff = '';
                            }
                            if (isset($data_staff[1])) {
                                $login_staff = $data_staff[1];
                            } else {
                                $login_staff = '';
                            }
                            if (isset($data_staff[2])) {
                                $sapas_staff = $data_staff[2];
                            } else {
                                $sapas_staff = '';
                            }
                            if (isset($data_staff[3])) {
                                $passw_staff = $data_staff[3];
                            } else {
                                $passw_staff = '';
                            }
                            if (isset($data_staff[4])) {
                                $access_staff = $data_staff[4];
                            } else {
                                $access_staff = '';
                            }


                            if (isset($data_staff[5])) {
                                $name_staff = $data_staff[5];
                            } else {
                                $name_staff = '';
                            }
                            if (isset($data_staff[6])) {
                                $email_staff = $data_staff[6];
                            } else {
                                $email_staff = '';
                            }
                            if (isset($data_staff[7])) {
                                $email_display_staff = $data_staff[7];
                            } else {
                                $email_display_staff = '';
                            }
                            if (isset($data_staff[8])) {
                                $phone_staff = $data_staff[8];
                            } else {
                                $phone_staff = '';
                            }
                            if (isset($data_staff[9])) {
                                $phone_display_staff = $data_staff[9];
                            } else {
                                $phone_display_staff = '';
                            }
                            if (isset($data_staff[10])) {
                                $post_staff = $data_staff[10];
                            } else {
                                $post_staff = '';
                            }
                            if (isset($data_staff[11])) {
                                $description_staff = $data_staff[11];
                            } else {
                                $description_staff = '';
                            }
                            if (isset($data_staff[12])) {
                                $photo_staff = $data_staff[12];
                            } else {
                                $photo_staff = '';
                            }
                            if (isset($data_staff[13])) {
                                $photo_display_staff = $data_staff[13];
                            } else {
                                $photo_display_staff = '';
                            }
                            if (isset($data_staff[14])) {
                                $active_staff = $data_staff[14];
                            } else {
                                $active_staff = '';
                            }
